...
for k, x, y in walk:
    ...
    ax.annotate(f"{k}G", (x, y), textcoords="offset points",
                xytext=(7, 5), fontsize=11, color="darkred", weight="bold")

ax.set_title(f"walk G -> 2G -> ... -> {STEPS}G on the point cloud\n"
             f"(curve y^2=x^3+7 mod {p}, G={G})", fontsize=12)
